proc countpgopts { bm } {
    ...
    if { $bm eq "TPC-C" } {
        ...
        set pgoptsfields [ dict create connection {pg_host {.countopt.c1.e1 get} pg_port {.countopt.c1.e2 get} pg_sslmode $pg_sslmode pg_azure_citus $pg_azure_citus pg_citus_loadbalancer $pg_citus_loadbalancer pg_citus_direct_workers $pg_citus_direct_workers} tpcc {pg_superuser {.countopt.c1.e3 get} pg_superuserpass {.countopt.c1.e4 get} pg_defaultdbase {.countopt.c1.e5 get}} ]
    } else {
        set pgoptsfields [ dict create connection {pg_host {.countopt.c1.e1 get} pg_port {.countopt.c1.e2 get} pg_sslmode $pg_sslmode pg_azure_citus $pg_azure_citus pg_citus_loadbalancer $pg_citus_loadbalancer pg_citus_direct_workers $pg_citus_direct_workers} tpch {pg_tpch_superuser {.countopt.c1.e3 get} pg_tpch_superuserpass {.countopt.c1.e4 get} pg_tpch_defaultdbase {.countopt.c1.e5 get}} ]
    }
...
proc configpgtpcc {option} {
    ...
    bind .tpc.c1.e9c <Button> {
        if { $pg_oracompat == "true" } {
            ...
            if { $pg_superuser eq "postgres" } { set pg_superuser "citus" }
	    ...
	    if { ".tpc.f1.e11a" in [ info commands ] } { .tpc.f1.e11a configure -state disabled }
            if { ".tpc.c1.e9f" in [ info commands ] } { .tpc.c1.e9f configure -state normal }
        } else {
	    ...
	    if { ".tpc.f1.e11a" in [ info commands ] } { .tpc.f1.e11a configure -state normal }
            ...
            if { ".tpc.c1.e9f" in [ info commands ] } {
                set pg_citus_direct_workers "false"
                .tpc.c1.e9f configure -state disabled
            }
        }
    }
    ...
    set Prompt $Parent.c1.p9f
    ttk::label $Prompt -text "Citus Direct Worker Loading :"
    set Name $Parent.c1.e9f
    ttk::checkbutton $Name -text "" -variable pg_citus_direct_workers -onvalue "true" -offvalue "false"
...
proc configpgtpch {option} {
    ...
    if { $option eq "all" || $option eq "build" } {
        ...
        foreach item {1} {
            ...
            ttk::radiobutton $Name -variable pg_scale_fact -text $item -value $item -width $width
...
proc metpgopts {} { 
    ...
    if { $bm eq "TPC-C" } {
        ...
        set pgoptsfields [ dict create connection {pg_host {.metric.c1.e1 get} pg_port {.metric.c1.e2 get} pg_sslmode $pg_sslmode pg_azure_citus $pg_azure_citus pg_citus_loadbalancer $pg_citus_loadbalancer pg_citus_direct_workers $pg_citus_direct_workers} tpcc {pg_superuser {.metric.c1.e3 get} pg_superuserpass {.metric.c1.e4 get} pg_defaultdbase {.metric.c1.e5 get}} ]
    } else {
        set pgoptsfields [ dict create connection {pg_host {.metric.c1.e1 get} pg_port {.metric.c1.e2 get} pg_sslmode $pg_sslmode pg_azure_citus $pg_azure_citus pg_citus_loadbalancer $pg_citus_loadbalancer pg_citus_direct_workers $pg_citus_direct_workers} tpch {pg_tpch_superuser {.metric.c1.e3 get} pg_tpch_superuserpass {.metric.c1.e4 get} pg_tpch_defaultdbase {.metric.c1.e5 get}} ]
    }
    if { $bm eq "TPC-C" } {
        if { $pg_oracompat eq "true" } {
            if { $pg_port eq "5432" } { set pg_port "5444" }
